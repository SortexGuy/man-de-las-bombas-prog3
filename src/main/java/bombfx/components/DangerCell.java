 ...
 * Las celdas de peligro son celdas especiales que infligen daño al jugador y eliminan enemigos cercanos.
 */
public class DangerCell extends EmptyCell {
    ...
    public DangerCell(Point2D pos, Player player, ArrayList<Enemy> enemies, Level level) {
        ...
        this.level = level;
        // Ajustar el rectángulo de colisión de la celda de peligro para que sea más pequeño que el de una celda vacía
        this.collRect = new Rectangle(pos.getX() + 4, pos.getY() + 4, SIZE - 8, SIZE - 8);
    }

    /**
     * Método para actualizar el estado de la celda de peligro en cada fotograma del juego.
    ...
    public void update(double delta) {
        ...
        // Si el tiempo restante es menor o igual a cero, la celda de peligro desaparece del nivel
        if (timeLeft <= 0) {
     ...
     * @param gContext El contexto gráfico en el que se dibujará la celda de peligro.
     */
    @Override
    public void draw(GraphicsContext gContext) {
        super.draw(gContext);

        // Dibujar el indicador de peligro (cuatro pequeños rectángulos rojos en las esquinas de la celda de peligro)
        int realSize = SIZE / 5;
    ...
    /**
     * Método para verificar si la celda de peligro colisiona con un rectángulo dado.
     * @param rect El rectángulo con el que se comprueba la colisión.
     * @return Siempre devuelve falso ya que una celda de peligro no puede colisionar.
     */
    @Override
